    ...
    Statement = [
      {
        ...
        Resource = [
          aws_sqs_queue.session_on_sale.arn,
          aws_sqs_queue.session_closed.arn
        ]
      }
    ]
  })
  
  depends_on = [aws_sqs_queue.session_on_sale, aws_sqs_queue.session_closed]
}
    ...
    Statement = [
      {
        ...
        Resource = "${aws_scheduler_schedule_group.ticketly.arn}/*"
      },
      {
        Effect = "Allow",
        Action = "iam:PassRole",
