...
describe('Event Ticketing System - Full Lifecycle Test', () => {
  ...
  test('should execute the full create, approve, order, and cleanup flow', async () => {
    ...
    await wait(STEP_INTERVAL); // Wait after status change
    ...
    await makeAuthenticatedRequest('delete', `${config.eventCommandServiceUrl}/v1/events/${eventId}`, userToken); // Use user token for deletion
    await wait(STEP_INTERVAL); // Wait after event deletion
    ...
    await wait(STEP_INTERVAL); // Wait after organization deletion
